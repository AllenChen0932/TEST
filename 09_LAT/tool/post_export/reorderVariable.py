...
def reorderVariable(varlist, reqlist):
    ...
    for req in reqlist:
        for var in varlist:
            if req in var.strip().split():
                varout.append(var)
                # varlist.remove(var)  # must NOT do this!
                # print(varlist)
                break
    # print(varout)
    if not varout:
        varout = varlist  # e.g., varlist = ['brs1_Mxy_seg at 0 deg', 'brs1_Mxy_seg at 15 deg', 'brs1_Mxy_seg at 30 deg', ...]
    return varout
