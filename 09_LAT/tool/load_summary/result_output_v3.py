...
from tool.load_summary.nacelle_acceleration import WriteNacelleAcc as nacelleacc
...
2021.1.15_v3.0.0 add nacelle acceleration statistics and update pitch bearing life
'''
...
class excel_operation:
        ...
        for i in range(lp_num):
            ...
            if self.ldd:
                ...
                if os.path.isdir(ldd_path):
                    ...
                    print('No results under %s!' %ldd_path)
                    self.loop_chan_pj[self.loopname[i]] = None
                    self.loop_prj_ext[self.loopname[i]] = None
                    self.loop_prj_var[self.loopname[i]] = None
                    self.lp_ch_pj_var[self.loopname[i]] = None
            ...
            if self.lrd:
                ...
                if os.path.isdir(lrd_path):
                    ...
                    print('No results under %s!' %lrd_path)
                    self.loop_chan_pj_lrd[self.loopname[i]] = None
                    self.loop_prj_ext_lrd[self.loopname[i]] = None
                    self.loop_prj_var_lrd[self.loopname[i]] = None
                    self.lp_ch_pj_var_lrd[self.loopname[i]] = None
    ...
    def write_result(self):
        ...
        # ********************************* LDD ***********************************************
        if self.ldd:
            print('Begin to write ldd...')
            sheet_name = table.sheetnames

            if 'LDD' in sheet_name:
                sheet = table.get_sheet_by_name('Gearbox LDD')
                table.remove(sheet)
            sheet_ldd = table.create_sheet('Gearbox LDD')

            col_start = 1
            # write result
            for i, lp_name in enumerate(self.loopname):
                ...
                var  = [_ for _ in var_list if 'Mx' in _][0]
                ...
                # header
                if len(pj_list) == 1:
                    sheet_ldd.cell(row=row_start+3, column=col_start, value=var)
                    sheet_ldd.cell(row=row_start+3, column=col_start+1, value='Time(s)')
                else:
                    sheet_ldd.cell(row=row_start+3, column=col_start, value=str(pj_name))
                    sheet_ldd.cell(row=row_start+4, column=col_start, value=var)
                    sheet_ldd.cell(row=row_start+4, column=col_start+1, value='Time(s)')

                # data
                for l in range(data_len):
                    if len(pj_list) == 1:
                        sheet_ldd.cell(row=row_start+3+l+1, column=col_start, value=data[l, 0]/1000)
                        sheet_ldd.cell(row=row_start+3+l+1, column=col_start+1, value=data[l, 1])
                    else:
                        sheet_ldd.cell(row=row_start+4+l+1, column=col_start, value=data[l, 0]/1000)
                        sheet_ldd.cell(row=row_start+4+l+1, column=col_start+1, value=data[l, 1])

                col_start += chan_col_max+3
                print('loop %s is done!' % (i+1))
            ...
            line_chart.title = 'Hub Mx LDD'  # 图的标题
            line_chart.style = 5  # 线条的style
            # line_chart.scatterStyle = 'marker'
            line_chart.y_axis.title = 'Time(s)'  # y坐标的标题
            line_chart.x_axis.title = "Mx(KNm)"  # x坐标的标题

            row_max = sheet_ldd.max_row
            for i in range(lp_num):
                lp_name = sheet_ldd.cell(1, 3*i+1).value

                yvalues = Reference(sheet_ldd, min_col=3*i+2, min_row=5, max_row=row_max)  # 图像的数据 起始行、起始列、终止行、终止列
                xvalues = Reference(sheet_ldd, min_col=3*i+1, min_row=5, max_row=row_max)
            ...
            cell_chart = 'A6'
            # print(cell_chart)
            sheet_ldd.add_chart(line_chart, cell_chart)  # 将图表添加到 sheet中
            print('ldd is done!\n')
        # ********************************* LDD ***********************************************
        ...
        if self.lrd:
            ...
            if 'LRD' in sheet_name:
                sheet = table.get_sheet_by_name('Pitch LRD')
                table.remove(sheet)
            sheet_lrd = table.create_sheet('Pitch LRD')
                    ...
                    for j in range(pj_num):
                        ...
                        for k in range(var_len):
                            ...
                            if pj_num == 1:
                                ...
                                sheet_lrd.cell(row=row_start+3, column=col_start+2*chan_var_num+2*k+1,
                                               value='Revs=%.1f' %rev_sum)
                            else:
                                sheet_lrd.cell(row=row_start+3, column=col_start+2*chan_var_num+2*k+1,
                                               value=str(pj_name))
                                sheet_lrd.cell(row=row_start+4, column=col_start+2*chan_var_num+2*k, value=var)
                                sheet_lrd.cell(row=row_start+4, column=col_start+2*chan_var_num+2*k+1,
                                               value='Revs=%.1f' %rev_sum)

                            # data
                            for l in range(data_len):
                                if pj_num == 1:
                                    sheet_lrd.cell(row=row_start+3+l+1, column=col_start+2*chan_var_num+2*k,
                                                   value=data[l, 0])
                                    sheet_lrd.cell(row=row_start+3+l+1, column=col_start+2*chan_var_num+2*k+1,
                                                   value=data[l, 1])
                                else:
                                    sheet_lrd.cell(row=row_start+4+l+1, column=col_start+2*chan_var_num+2*k,
                                                   value=data[l, 0])
                                    sheet_lrd.cell(row=row_start+4+l+1, column=col_start+2*chan_var_num+2*k+1,
                                                   value=data[l, 1])
            ...
            # 作图
            line_chart = ScatterChart()
            line_chart.title = 'Pitch Travel'  # 图的标题
            line_chart.style = 5  # 线条的style
            # line_chart.scatterStyle = 'marker'
            line_chart.y_axis.title = 'Revs'  # y坐标的标题
            line_chart.x_axis.title = "Mx(KNm)"  # x坐标的标题

            row_max = sheet_lrd.max_row
            for i in range(lp_num):
                lp_name = sheet_lrd.cell(1, 3*i+1).value

                yvalues = Reference(sheet_lrd, min_col=3*i+2, min_row=5, max_row=row_max)  # 图像的数据 起始行、起始列、终止行、终止列
                xvalues = Reference(sheet_lrd, min_col=3*i+1, min_row=5, max_row=row_max)
                series = Series(yvalues, xvalues, title=lp_name)
                line_chart.series.append(series)
            ...
            cell_chart = 'A6'
            # print(cell_chart)
            sheet_lrd.add_chart(line_chart, cell_chart)  # 将图表添加到 sheet中
            print('lrd is done!\n')
        # ********************************* LRD ***********************************************
        ...
        if self.mbl:
            for i in range(lp_num):
                ...
                if self.mbl_list[i]:
                    ...
                    print('loop %s has no results!' %(i+1))
        ...
        if self.nac:
            for i in range(lp_num):
                print('Begin to write nacelle acceleration %s' % self.loopname[i])
                if self.nac_list[i]:
                    nacelleacc(self.nac_list[i], table, self.loopname[i], i)
                    print('loop %s is done!' % (i+1))
                else:
                    print('loop %s has no results!' %(i+1))
