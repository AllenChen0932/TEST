...
class Ultimate(object):
    ...
    def __init__(self,
        ...
        self.cha_dict  = cha_dict    #{channel: variable list}, e.g. yb:[Yaw bearing Fx, Yaw bearing Fy]
        self.dlc_list  = dlc_list
        self.iec_stdard = iec_stdard
        self.etm_option = etm_option

        if not os.path.exists(self.ult_path):
            os.makedirs(self.ult_path)
            # print(self.ult_path)
        ...
        self.dlc_sf = {'DLC11':1.25,
                       ...
                       'DLC83':1.0}
        # IEC61400-1 ED3
        if self.iec_stdard == '1':
            self.dlc_sf.update({'DLC81': 1.5})
                ...
                if os.path.isdir(lc_path):
                    ...
                    if self.mean_index in lc:
                        ...
                        if group_name not in self.subgroup_list:
                            ...
                            self.allgroup_dlc[group_name]   = dlc
            ...
            if len(value) == 2:
                ...
                for sec in value[1]:
                    ...
                    for gp in self.allgroup_list:
                        ...
                        content += 'TYPE	0\n'
                        if self.include_sf:
                            lc = self.allgroup_dlc[gp]
                            print(lc)
                            if lc not in ['DLC21b','DLC21c']:
                                sf = self.dlc_sf[lc[:5]]
                            else:
                                sf = 1.1
                        content += 'SF	 %s\n' %sf
                    ...
                    pj_path = os.path.join(self.ult_path, pj_name).replace(' ', '_')
                    # print(pj_path)
                ...
                for gp in self.allgroup_list:
                    ...
                    if self.include_sf:
                        ...
                        if lc not in ['DLC21b', 'DLC21c']:
                            ...
                            sf = 1.1
                    content += 'SF	 %s\n' %sf
                    # print(self.allgroup_dlc[gp],self.dlc_sf[self.allgroup_dlc[gp]])
                ...
                # pj_path = os.path.join(self.ult_path, file_name).replace(' ', '_')
            ...
            if len(value) == 2:
                ...
                for sec in value[1]:
                    ...
                    content += 'MSTART MAXMIN\n' \
                               'HISTTYPE	%s\n' %(2 if key=='tc' else 1)
                    content += 'NCASES\t%s\n' %self.dlc_number
                    content += ('NUMSUBGROUP	%s\n' %len(self.subgroup_list)) if self.subgroup_list else ''
                    content += ('SUBGROUPTYPE %s\n' %' '.join([self.allgroup_types[lc] for lc in self.subgroup_list]))\
                        if self.subgroup_list else ''
                    ...
                    for j in range(var_num):
                        ...
                        for ind, lc in enumerate(self.all_loadcase):
                            if self.include_sf:
                                dlc = self.allcase_dlc[lc]
                                print(dlc)
                                if dlc not in ['DLC21b', 'DLC21c']:
                                    sf = self.dlc_sf[dlc[:5]]
                                else:
                                    sf = 1.1
    ...
    def write_pj_variable(self, channel, var_list, height=None):
        ...
        if channel == 'br':
            content += 'NVARS	%s\n' %len(var_list)
            content += 'BLADEFLAG	%s\n' %('-1' if len(var_list)==24 else '0')
            content += '%s' %('COMBINE	3\n' if len(var_list)==24 else '')
            ...
            for index,var in enumerate(var_list):
                ...
                content += 'ATTRIBF	%'+'%s\n' %(int(15+index%3) if len(var_list)==24 else 15)
                content += 'VARIAB	"%s"\n' %var
            ...
            for index, var in enumerate(var_list):
                ...
                content += 'ATTRIBF	%'+'%s\n' %(int(41+index%3) if len(var_list)==24 else 41)
                content += 'VARIAB	"%s"\n' %var
                content += 'DESCRIPTION	"%s, Distance along blade= %sm"\n' %(var, height)
                content += 'NDIMENS	3\n'
                content += 'DIMFLAG	-1\n'
            ...
            for index, var in enumerate(var_list):
                ...
                content += 'ATTRIBF	%'+'%s\n' %(int(59+index%3) if len(var_list)==24 else 59)
                content += 'VARIAB	"%s"\n' %var
                content += 'DESCRIPTION	"%s, Distance along blade= %sm"\n' %(var, height)
                content += 'NDIMENS	3\n'
                content += 'DIMFLAG	-1\n'
            ...
            for index, var in enumerate(var_list):
                ...
                content += 'ATTRIBF	%'+'%s\n' %(int(62+index%3) if len(var_list)==24 else 62)
                content += 'VARIAB	"%s"\n' %var
                content += 'DESCRIPTION	"%s, Distance along blade= %sm"\n' %(var, height)
                content += 'NDIMENS	3\n'
                content += 'DIMFLAG	-1\n'
            ...
            content += 'NVARS	%s\n' % len(var_list)
            content += 'BLADEFLAG	%s\n' %('-1' if len(var_list)==9 else '0')
            content += '%s' %('COMBINE	3\n' if len(var_list)==9 else '')
            content += '%s'%('COMBINEMETHOD	0\n' if len(var_list)==9 else '')

            ext_list = (['22']*6 + ['18', '19', '20']) if len(var_list)==9 else (['22']*2 + ['18'])
            for index, var in enumerate(var_list):
