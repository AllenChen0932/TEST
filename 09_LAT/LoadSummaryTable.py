...
from tool.load_summary.result_output_v2 import excel_operation as new
...
class LoadSumWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.gbox2 = QGroupBox('Main Function')
        ...
        # self.checkbox8 = QCheckBox('Fatigue per yaw error')

        self.glayout3 = QGridLayout()
        self.glayout3.addWidget(self.cb_fatwind, 0, 0, 1, 1)
        self.glayout3.addWidget(self.cb_fatdlc,  1, 0, 1, 1)
        self.glayout3.addWidget(self.cb_fatwave, 2, 0, 1, 1)
        # self.glayout3.addWidget(self.checkbox8, 3, 0, 1, 1)
        ...
        self.glayout4.addWidget(self.cb_plotldd, 1, 0, 1, 1)
        self.glayout4.addWidget(self.cb_plotlrd, 2, 0, 1, 1)
        self.glayout4.addWidget(self.cb_dynpowc, 3, 0, 1, 1)
        self.glayout4.addWidget(self.cb_aepdlc12, 4, 0, 1, 1)
        self.glayout4.addWidget(self.cb_towercl, 0, 1, 1, 1)
        self.glayout4.addWidget(self.cb_nacacce, 1, 1, 1, 1)
        self.glayout4.addWidget(self.cb_gearequ, 3, 1, 1, 1)
        self.glayout4.addWidget(self.cb_pitbear, 2, 1, 1, 1)
        self.glayout4.addWidget(self.cb_mainber, 4, 1, 1, 1)
    ...
    def load_setting(self):
        ...
        if config.has_section('Pitch Bearing Life'):
            self.pbl_m = config.get('Pitch Bearing Life', 'm')
    ...
    def check_input(self):
        ...
        if self.xls_temp:
            try:
                ...
                self.loopname = [sheet[i][0].value for i in range(2,row_max+1)
                                 if sheet[i][0].value!=None]
                self.pst_list = [sheet[i][2].value for i in range(2,row_max+1)
                                 if (sheet[i][1].value == 'Post') and sheet[i][2].value]
                self.dlc_list = [sheet[i][2].value for i in range(2,row_max+1)
                                 if (sheet[i][1].value == 'DLC12') and sheet[i][2].value]
                self.lct_list = [sheet[i][2].value for i in range(2,row_max+1)
                                 if (sheet[i][1].value == 'LCT') and sheet[i][2].value]
                self.ult_list = [sheet[i][2].value for i in range(2,row_max+1)
                                 if (sheet[i][1].value == 'Ultimate') and sheet[i][2].value]
                self.fat_list = [sheet[i][2].value for i in range(2,row_max+1)
                                 if (sheet[i][1].value == 'Rainflow') and sheet[i][2].value]

                print('Loop name:', self.loopname)
                print('Ultimate :', self.ult_list)
                print('Fatigue  :', self.fat_list)
                print('Post     :', self.pst_list)
                print('DLC12    :', self.dlc_list)
    ...
    def handle(self):
        ...
        self.ultimate = True if self.cb_ultimate.isChecked() else False
        self.fatigue  = True if self.cb_fatigue.isChecked() else False
        self.fat_wind = True if self.cb_fatwind.isChecked() else False
        self.heat_map = True if self.cb_heatmap.isChecked() else False
        self.fat_case = True if self.cb_fatdlc.isChecked() else False
        self.aep_dlc  = True if self.cb_aepdlc12.isChecked() or self.cb_dynpowc.isChecked() else False
        self.plot_lrd = True if self.cb_plotlrd.isChecked() else False
        self.plot_ldd = True if self.cb_plotldd.isChecked() else False
        self.main_bl  = True if self.cb_mainber.isChecked() else False
        self.pitch_bl = True if self.cb_pitbear.isChecked() else False
        self.gearbox  = True if self.cb_gearequ.isChecked() else False
